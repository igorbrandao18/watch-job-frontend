      ...
      <div class="login-center">
        <v-card class="login-card-glass animate-card-in" max-width="380" elevation="24">
          <div class="text-center mb-7">
            ...
            <h1 class="text-h5 font-extrabold mb-1 text-white tracking-wide animate-title">Entrar</h1>
            <span class="text-body-2 text-grey-lighten-1">Acesse sua conta para continuar</span>
          </div>
          <v-form @submit="handleLogin" class="login-form">
            <v-text-field
              v-model="email"
              label="E-mail"
              type="email"
            ...
            <v-btn :loading="loading" :disabled="loading" type="submit" color="yellow" class="login-btn-anim force-yellow" size="large" elevation="12" block>
              ...
              <span v-else class="d-flex align-center"><v-progress-circular indeterminate color="primary" size="20" class="mr-2" />Entrando...</span>
            </v-btn>
            <div v-if="error" class="text-red text-center text-caption mt-2 animate-error">{{ error }}</div>
          </v-form>
          <div class="mt-7 text-center">
            <span class="text-grey-lighten-1">Novo por aqui?</span>
            <router-link to="/register" class="ml-2 text-secondary font-bold hover:underline">Quero ter Watch</router-link>
          </div>
        </v-card>
      </div>
    </div>
